...
func start_logging(log_file_path: String, match_info: Dictionary = {}) -> void:
	...
	if not _logger:
		_logger = InternalLogger.new(self)
	else:
		_logger.stop()
	...
	if not mechanized_replay:
		...
		if _ticks_spent_regaining_sync > 0:
			...
			if not _cleanup_buffers():
				...
				if _logger:
					_logger.skip_tick(InternalLogger.SkipReason.INPUT_BUFFER_UNDERRUN, start_time)
				return
			...
			if min_lag_to_regain_sync > 0 and _calculate_max_local_lag() > min_lag_to_regain_sync:
				...
				_send_input_messages_to_all_peers()
				if _logger:
					_logger.skip_tick(InternalLogger.SkipReason.WAITING_TO_REGAIN_SYNC, start_time)
				return
		...
		if skip_ticks > 0:
			...
			if skip_ticks == 0:
				...
				_send_input_messages_to_all_peers()
				if _logger:
					_logger.skip_tick(InternalLogger.SkipReason.ADVANTAGE_ADJUSTMENT, start_time)
				return
