...
func _loader_thread_function(input: Array) -> void:
	...
	while not file.eof_reached():
		...
		if header == null:
			if data['log_type'] == InternalLogger.LogType.HEADER:
				header = data
